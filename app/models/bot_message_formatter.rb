...
class BotMessageFormatter
  ...
  def header
    if @rank.blank?
      ...
      make_header_thread(@rank)
    else
      make_header_channel(@rank)
    end
  end
  ...
  def embed_message
    if @rank.blank?
    ...
    else
      {
        ...
        description: set_description(@rank),
        url: set_url(@rank),
        color: 0x2727ff,
